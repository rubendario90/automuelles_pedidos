...
import { useAuth } from '../../../src/context/AuthContext'; // Ajusta la ruta según tu proyecto
...
const RevisionFinal = () => {
  ...
  // Solo habilita el botón si todos los productos están seleccionados
  const allChecked = documentDetails && documentDetails.length > 0 &&
    documentDetails.every((_, idx) => checkedItems[idx]);

  // Función para recargar la lista de facturas
  ...
  const fetchDocumentDetails = async (factura) => {
    ...
    try {
      ...
      setCheckedItems({}); // Reinicia los checkboxes
    } catch (err) {
  ...
  const handleConfirmSelection = async () => {
    ...
    if (!allChecked) {
      alert('Debes seleccionar todos los productos para entrega total.');
      return;
    }
    ...
    try {
      ...
      handleBackToFacturas();
      fetchPickingFacturas(); // Recarga la lista sin refrescar la página
    } catch (error) {
