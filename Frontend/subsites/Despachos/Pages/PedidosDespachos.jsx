...
import '../assets/Despachos.css';

const PedidosDespachos = () => {
  ...
  useEffect(() => {
    const fetchFacturasPendientes = async () => {
      try {
        const response = await axios.get('/api//facturas-pedidos-despachos', { withCredentials: true });
        setFacturasPendientes(response.data);

        // Para cada factura, consulta los detalles
        response.data.forEach(async (factura) => {
          try {
            const detailsRes = await axios.post('/api/document-details', {
            ...
            setDocumentDetails(prev => ({
              ...prev,
              [factura.factura_id]: {
                StrReferencia1: detailsRes.data[0]?.StrReferencia1 || '',
                ClienteNombre: detailsRes.data[0]?.ClienteNombre || '',
              }
            }));
          } catch (e) {
            setDocumentDetails(prev => ({
  ...
  const handleGestionarClick = (factura) => {
    navigate('/Despachos/gestionar-factura', { state: { factura } });
  };
                ...
                <div className="col-12 col-sm-6 col-md-4 mb-4" key={factura.id}>
                  <div className="card text-center shadow-sm">
                    ...
                    <div className="card-body">
                      ...
                      <p>
                        <strong>Enviar A:</strong> {documentDetails[factura.factura_id]?.StrReferencia1 || 'Cargando...'}
                      </p>
                      <p>
                        <strong>Cliente:</strong> {documentDetails[factura.factura_id]?.ClienteNombre || 'Cargando...'}
                      </p>
                      <p className="card-text">
...
export default PedidosDespachos;