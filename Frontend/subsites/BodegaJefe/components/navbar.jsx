...
const Navbar = () => {
  ...
  const menuItems = [
    { icon: <Home size={22} />, label: 'Inicio', view: 'Home', path: '/bodega-jefe' },
    { icon: <Home size={22} />, label: 'Revision Final', view: 'RevisionFinal', path: '/bodega-jefe/revision-final' },
    { icon: <Inbox size={22} />, label: 'Pedidos Parciales', view: 'PedidosParciales', path: '/bodega-jefe/pedidos-parciales' },
    {
      icon: (
  ...
  const changeRole = async () => {
    try {
      const response = await axios.put('/api/change-role', { newRole: 'JefeBodega' }, { withCredentials: true });
          ...
          {isSettingsOpen && (
            <ul className="dropdown-menu">
              ...
              <li
                ...
                onClick={() => navigate('/bodega-jefe/historial')}
              >
                Historial
              </li>
