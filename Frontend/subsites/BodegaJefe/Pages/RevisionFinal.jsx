...
const RevisionFinal = () => {
  ...
  const { user } = useAuth(); // user.id es el id del usuario autenticado
  const [facturas, setFacturas] = useState([]);
  const [selectedFactura, setSelectedFactura] = useState(null);
  const [documentDetails, setDocumentDetails] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [checkedItems, setCheckedItems] = useState({}); // Solo una declaración aquí
  ...
  const fetchDocumentDetails = async (factura) => {
    ...
    try {
      ...
      setCheckedItems({}); // Reinicia los checkboxes al ver detalles
    } catch (err) {
